...
sub  NutzerListe {
  ...
  while( my $q = $sql_ref->handle->fetch ) {
        ...
        my $sql="select a.db_unit, a.ext_unit, a.ext_id, b.db_unit, b.ext_unit, b.ext_id, c.db_address, c.ext_address 
                from unit a left outer join unit b on a.db_member=b.db_unit left outer join address c on a.db_address=c.db_address 
                where a.user_id=$q->[0]";
...
';
    ...
    $self->{'_longtablecontent'} .= "
                    ...
                    \\chead{\\textbf{ \\raisebox{1ex}{NutzerListe}} }
    ...
    \\rowcolor{mg}{\\textbf{\\makecell{User (user\\_id) \\\\
                                       Sprache/Disabled/Status/Typ \\\\ 
                                       Letztes Login \\\\ 
                                       SessionID}}} &
                  {\\textbf{\\makecell{ Unit (db\\_unit)/ \\ Mitglied von (db\\_member)/ \\\\ 
                                        AddressID (db\\_address)}}} \\\\ 
    \\hline";
    ...
    foreach my $adr ( @{$data} ) {
            ...
            $self->{'_longtablecontent'} .=$daten[0].'&'.$daten[1]."\\\\ \\hline";    
    }
