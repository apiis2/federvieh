...
CREATE OR REPLACE FUNCTION user_get_ext_id_animal(int) RETURNS text AS $$ select case when ext_unit='bundesring' then  ext_animal else ext_id || '-' || ext_animal end  from entry_unit c inner join entry_transfer d on c.db_unit=d.db_unit where d.db_animal=$1 order by c.ext_unit limit 1; $$ LANGUAGE SQL;
